...
class FeudalRunner(BaseRunner):
    ...
    def run_batch(self, train_summary):
        ...
        for n in range(self.n_steps):
            ...
            # 針對每個 timestep，取出 score_cumulative 並經過 scale_reward 處理
            scores = [t.observation["score_cumulative"][0] for t in obs_raw]
            scaled_rewards = [scale_reward(score) for score in scores]
            rewards[n, :] = scaled_rewards
...
def compute_returns_and_advantages(rewards, dones, values, s, goals, discount, T, nenvs, c):
    ...
    for t in reversed(range(T)):
        ...
        # 動態標準化 worker term
        current_mean = np.mean(wrk_term)
        current_std = np.std(wrk_term)
        # 將該 timestep 的 worker term 進行正規化並 clip 到 [-1,1]
        wrk_term_norm = (wrk_term - current_mean) / (current_std + epsilon)
        wrk_term_norm = np.clip(wrk_term_norm, -1, 1)
        
        # 用經過動態標準化的 worker term 來計算返回值
        returns[t, :] = mgr_rewards + mgr_term
        returns_intr[t, :] = wrk_rewards + wrk_term_norm
        ...
        debug_worker_terms.append(wrk_term_norm)
