...
def compute_returns_and_advantages(rewards, dones, values, s, goals, discount, T, nenvs, c):
    ...
    for t in reversed(range(T)):
        epsilon = 1e-8
        returns[t,:] = rewards[t+c,:] + manager_discount * returns[t+1,:] * (1-dones[t+c,:]) + epsilon
        # returns[t,:] = manager_discount * returns[t+1,:] * (1-dones[t+c,:])
        returns_intr[t,:] = rewards[t+c,:] + alpha * r_i[t,:] + discount * returns_intr[t+1,:] * (1-dones[t+c,:]) + epsilon
    returns = returns[:-1,:]
